...
```JAVA
// OJ: https://leetcode.com/problems/reorder-routes-to-make-all-paths-lead-to-the-city-zero/
    ...
    public int minReorder(int n, int[][] connections) {
        ...
        while (set.size() != n) {
            for (int[] connect : connections) {
                //check each node to see if the second element in the list, if not have to add and result++
                if (set.contains(connect[0]) && !set.contains(connect[1])) {
                    result++;
                    set.add(connect[1]);
                } else if (set.contains(connect[1]) && !set.contains(connect[0])){
                    set.add(connect[0]);
                }
            }
        }
