...
**Companies**:  
[Facebook](https://leetcode.com/company/facebook), [Microsoft](https://leetcode.com/company/microsoft), [Amazon](https://leetcode.com/company/amazon), [Bloomberg](https://leetcode.com/company/bloomberg), [Apple](https://leetcode.com/company/apple)
...
class Solution {
    public Node cloneGraph(Node node) {
        ...
        //create the map to record each node that is copied
        Map<Integer, Node> copies = new HashMap<>();
        return cloneNode(node, copies);
    }
    private static Node cloneNode(Node node, Map<Integer, Node> copies)  {
        if (copies.containsKey(node.val)) {
            return copies.get(node.val);
        } 
        Node newNode = new Node(node.val);
        copies.put(node.val, newNode);
        for (Node neighbor : node.neighbors) {
            newNode.neighbors.add(cloneNode(neighbor, copies));
        }
        return newNode;
    }
}
